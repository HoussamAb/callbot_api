    ...
    def get(self, request):
        if request.GET.get("action"):
            ...
            if request.GET.get("action") == "train":
                ...
                return render(request, template, prompt)
            # ~~~~~~~~~~~~~~~~~~~~~~~~ TEST PAGE ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ #
            if request.GET.get("action") == "algo":
                trained = statModel.objects.filter(Score__isnull=False)
                non_trained = statModel.objects.filter(Score__isnull=True)
                from django.core import serializers
                tmpJson = serializers.serialize("json", trained)
                tmpJson2 = serializers.serialize("json", non_trained)
                ...
                # GET request returns the value of the data with the specified key.
                return HttpResponse(tmpObj)
            # ~~~~~~~~~~~~~~~~~~~~~~~~ TEST PAGE ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ #
            if request.GET.get("action") == "files":
            ...
            if request.GET.get("action") == "datasets":
                ...
                prompt = {
                    'dataset': df_dataset.to_json(force_ascii=True),
                }
                # GET request returns the value of the data with the specified key.
                return JsonResponse(prompt, status=201)
